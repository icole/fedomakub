...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
  ...
  INSTALLER_FILE="$FEDOMAKUB_PATH/install/desktop/optional/app-$INSTALLER.sh"

  source $INSTALLER_FILE && gum spin --spinner globe --title "Install completed!" -- sleep 3
fi

clear
source $FEDOMAKUB_PATH/bin/fedomakub-sub/header.sh
source $FEDOMAKUB_PATH/bin/fedomakub-sub/install.sh
